...
describe("Codex Pets vs VS Code Pets guide", () => {
  it("renders the maintainer byline and the first-hand methodology section", () => {
    ...
    expect(source).toContain("VS_VSCODE_PETS_QUERY_EXAMPLES");
  ...
  it("ranks example pets against the full approved catalog", () => {
    ...
    const routeSource = readFileSync(
      new URL("../codex-pets-vs-vscode-pets.md/route.ts", import.meta.url),
      "utf8",
    );
  ...
  it("keeps the markdown route on the shared guide source", () => {
    ...
    expect(source).toContain("buildVsVsCodePetsGuideMarkdown");
  });
