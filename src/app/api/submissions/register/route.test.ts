...
describe("POST /api/submissions/register", () => {
  ...
  it("rejects an explicitly empty anonymous author name when contact email is set", async () => {
    vi.mocked(getCurrentPrincipal).mockResolvedValueOnce(null);
    const formData = validSubmissionForm();
    formData.set("publicAuthorName", "");
  ...
  it.each(["anon@example.com", " MiXeD@Example.com ", "alias+tag@example.com"])(
    "defaults an omitted anonymous alias for legacy clients: %s",
    async (email) => {
      vi.mocked(getCurrentPrincipal).mockResolvedValueOnce(null);
      mockSuccessfulSubmission("pet_legacy_alias");
      const form = validSubmissionForm();
      form.set("contactEmail", email);
      form.delete("publicAuthorName");

      const response = await POST(submissionRequest(form));

      expect(response.status).toBe(201);
      expect(createPendingPet).toHaveBeenCalledWith(expect.objectContaining({
        ownerName: email.trim().split("@")[0],
        publicEmailRequested: false,
      }));
    },
  );

  it("replays an omitted alias as the equal explicit default alias", async () => {
    vi.mocked(getCurrentPrincipal).mockResolvedValue(null);
    mockSuccessfulSubmission("pet_default_alias_replay");
    const legacy = validSubmissionForm();
    legacy.delete("publicAuthorName");
    const first = await POST(submissionRequest(legacy, "default-alias-replay"));
    const second = await POST(submissionRequest(validSubmissionForm(), "default-alias-replay"));
    expect(first.status).toBe(201);
    expect(second.status).toBe(201);
    expect(createPendingPet).toHaveBeenCalledOnce();
  });
  ...
  it("does not replace a missing signed-in email with client contact data", async () => {
    vi.mocked(getCurrentPrincipal).mockResolvedValueOnce({
      userId: "user_without_email", email: null, name: "Profile name", role: "user",
    });
    mockSuccessfulSubmission("pet_without_account_email");
    const form = validSubmissionForm();
    form.set("contactEmail", "someone-else@example.com");
    form.set("publishContactEmail", "true");
    const response = await POST(submissionRequest(form));
    expect(response.status).toBe(400);
    expect(createPendingPet).not.toHaveBeenCalled();
  });
