...
export async function GET(): Promise<Response> {
  ...
  return new Response(
    [
      ...
      `- Embed page: ${toPublicUrl("/embed/{slug}")}. Minimal iframe-friendly HTML for an approved pet.`,
      "",
      "## Agent access",
      "",
      `- Manifest: ${toPublicUrl("/api/manifest")}`,
      `- MCP config: codex mcp add codexPets --url ${toPublicUrl("/mcp")}`,
      "- MCP tools: search_pets, get_pet, get_install_instructions, get_badge_code, get_embed_code.",
