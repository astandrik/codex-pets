...
export async function GET(): Promise<Response> {
  ...
  return new Response(
    [
      ...
      "Codex Pets is also an agent-readable registry. The public MCP endpoint and share routes expose approved read-only pet data plus read-only pet request workflow discovery.",
      "",
      "Important notes:",
      "",
      ...
      "",
      "## Recommended assistant tasks",
      "",
      "- Pick a pet by style.",
      "- Explain a pet detail page.",
      "- Generate install instructions.",
      "- Connect MCP.",
      "- Search pets through JSON, TOON, or MCP.",
      "- Draft a pet request.",
      "- Explain how to submit a pet.",
      "",
      "## Core pages",
      "",
      ...
      `- [Codex Pets vs OpenPets](${toPublicUrl("/guides/codex-pets-vs-openpets")}): First-hand comparison of the two Codex pet registries. Markdown: ${toPublicUrl("/guides/codex-pets-vs-openpets.md")}.`,
      ...
      `- [MCP Registry metadata](${toPublicUrl("/server.json")}): Public MCP Registry metadata for the remote server.`,
      `- [Well-known MCP Registry metadata](${toPublicUrl("/.well-known/mcp/server.json")}): Well-known MCP metadata mirror.`,
      ...
      `- Well-known MCP config: ${toPublicUrl("/.well-known/mcp")}`,
      "- MCP tools: search_pets, get_pet, get_install_instructions, get_badge_code, get_embed_code, get_card_code, get_pet_request_info.",
