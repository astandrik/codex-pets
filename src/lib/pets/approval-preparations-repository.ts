...
($preparation_id, $pet_id, $pet_slug, $pet_updated_at, $reviewer_id,
 ...
 $status, $attempts, $now, $empty, $empty, $empty, $now, $now);
  ...
  async function get(preparationId: string): Promise<ApprovalPreparation | null> {
    if (!dependencies.isConfigured()) return null;
...
WHERE preparation_id = $preparation_id
  AND status = $expected_status;
      ...
      return getWithExecute(execute, candidate.preparationId);
    });
  }
  ...
  async function markFailure(input: {
    ...
    return dependencies.transaction(async (execute) => {
      ...
      const nextAttemptAt = input.retryable
        ...
        : null;
      const nextStatus = nextAttemptAt ? "retry" : "manual_review";
  ...
  async function finalize(input: {
    ...
    return dependencies.transaction(async (execute) => {
      ...
      const currentInputRevision = await getInputRevisionWithExecute(
        execute,
        input.inputScope,
      );
      if (currentInputRevision !== input.expectedInputRevision) return false;
...
WHERE generation_id = $generation_id
  AND ranking_revision = $ranking_revision;
      ...
      const resultSets = (checks as { resultSets?: Array<{ rows?: unknown[] }> })
        .resultSets ?? [];
      const counts = resultSets.map((set) =>
        uintAt({ items: (set.rows?.[0] as { items?: [] } | undefined)?.items }, 0)
      );
      if (
        counts[0] !== 1 ||
        counts[1] !== 1 ||
...
WHERE annotation_revision = $annotation_revision ORDER BY pet_slug;
  ...
  async function updateOutcome(execute: Execute, input: {
    preparationId: string;
    status: ApprovalPreparationStatus;
    failureCode: string;
    nextAttemptAt: string;
...
function selectDeclarations(): string {
  return "DECLARE $preparation_id AS Utf8;";
}
...
async function withSerializableTransaction<T>(
  ...
  return withSession(async (session: Session) => {
    ...
    if (!transaction.id) throw new Error("Unable to start approval transaction.");
    ...
    try {
      ...
      try {
        ...
        // The transaction may already be aborted by YDB.
      }
      throw error;
    }
  });
