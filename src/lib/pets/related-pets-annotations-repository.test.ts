...
describe("related pet annotations repository", () => {
  it("reads, lists, and writes one immutable revision", async () => {
    ...
    await expect(repository.get("annotation-v11", "vi")).resolves.toMatchObject({
      slug: "vi",
      sourceHash: "hash",
      annotationText: "entity: vi",
    });
    await expect(repository.listByRevision("annotation-v11")).resolves.toHaveLength(1);
    await repository.upsert({
      annotationRevision: "annotation-v11",
      slug: "vi",
      sourceHash: "hash",
      proposalJson: "{}",
      annotationJson: "{}",
  ...
  it("is inert without YDB configuration", async () => {
    ...
    await expect(repository.get("v11", "vi")).resolves.toBeNull();
    await expect(repository.listByRevision("v11")).resolves.toEqual([]);
  });
});
