...
export function parseRelatedPetAnnotationProposal(input) {
  const value = strictObject(input, "annotation proposal", PROPOSAL_FIELDS);
  const entityValue = strictObject(value.entity, "entity", [
    "key",
    "aliases",
    "confidence",
    "evidence",
...
function parseResolvedProposalInput(input) {
  if (
    ...
    const value = strictObject(input, "annotation proposal", [
      "entity",
      "franchises",
      "franchiseFamilies",
      "collections",
...
function relationProposals(input, path) {
  ...
  for (const [index, item] of input.entries()) {
    const value = strictObject(item, `${path}[${index}]`, [
      "key",
      "confidence",
      "evidence",
    ]);
    const proposal = {
      key: canonicalKey(value.key, `${path}[${index}].key`),
      confidence: confidence(value.confidence, `${path}[${index}]`),
      evidence: evidenceList(value.evidence, `${path}[${index}].evidence`),
    };
...
function parseResolvedAnnotation(input) {
  const value = strictObject(input, "resolved annotation", [
    "schemaVersion",
    "entity",
    "aliases",
    "franchises",
  ...
  if (value.schemaVersion !== 1) {
    throw new Error("Resolved annotation schemaVersion must be 1.");
  }
  const annotation = {
    schemaVersion: 1,
...
function evidenceList(input, path) {
  if (!Array.isArray(input) || input.length > 4) {
    throw new Error(`${path} must contain at most 4 items.`);
  }
