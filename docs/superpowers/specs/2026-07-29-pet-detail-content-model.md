...
Rendering: server-rendered "Related pets" section after the main body on `/pets/[slug]`, rendered as the same gallery cards (`PetCard` in the shared `pet-grid`) — animated sprite preview, displayName, kind label, description, metrics. Data for the ≤4 selected slugs is hydrated through `listApprovedPetsBySlugs` (full `PublicPet`: sprite URLs, metrics, owner profile); the lightweight candidate query above stays for scoring only, and hydration failure degrades to no section. Non-approved pages (pending/rejected) render no section. The markdown twin keeps the lighter text-list form (displayName + kind + one-line description truncated at 120 code points) fed by the same selector.
